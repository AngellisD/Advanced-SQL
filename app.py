...
def prcp(date):
    result = list(np.ravel(session.query(Measurement.date, func.sum(Measurement.prcp)).\
...
def tobs():
    ...
    filter(Measurement.date==year_date).order_by(Measurement.date.desc()).all()
...
def list2(start, end):
    start_date = start
    end_date = end
    list2 = session.query(func.min(Measurement.tobs), func.avg(Measurement.tobs), func.max(Measurement.tobs)).\
